...
// wifi configuration
const char SSID[] = "EXCEED_LEFT_1_2.4GHz";
...
void loop() {
  ...
  */
  uint32_t now = millis();
  // get data from a piece of SKE
  if(now-last_sent_time>500){
    GET("http://ecourse.cpe.ku.ac.th/exceed/api/fingerplam-water/view",get_request,server_data.water);
    Serial.print("water: ");
    Serial.println(server_data.water);
    delay(200);
    GET("http://ecourse.cpe.ku.ac.th/exceed/api/fingerplam-fan/view",get_request,server_data.fan);
    Serial.print("fan: ");
    Serial.println(server_data.fan);
    delay(200);
    GET("http://ecourse.cpe.ku.ac.th/exceed/api/fingerplam-indoor_LED/view",get_request,server_data.indoor_LED);
    Serial.print("LEDOOOOOOOOOk: ");
    Serial.println(server_data.indoor_LED);
    delay(200);
  }
  ...
  while (se_read.available()) {
    ...
    if (cur_buffer_length == -1) {
      ...
      if (cur_buffer_length == expected_data_size) {
        switch (cur_data_header) {
          case UPDATE_PROJECT_DATA: {
            Serial.println("He want to update");
            ProjectData *project_data = (ProjectData*)buffer;
            POST(set_builder("fingerplam-indoor_humidity",project_data->indoor_humidity).c_str(),update_data_to_server_callback);
            delay(200);
            POST(set_builder("fingerplam-indoor_temperature",project_data->indoor_temperature ).c_str(),update_data_to_server_callback);
            delay(200);
            POST(set_builder("fingerplam-indoor_light",project_data->indoor_light).c_str(),update_data_to_server_callback);
            delay(200);
            POST(set_builder("fingerplam-faucet",10).c_str(),update_data_to_server_callback);
            delay(200);
            //POST(set_builder("fingerplam-fan",project_data->fan).c_str(),update_data_to_server_callback);
            //delay(200);
